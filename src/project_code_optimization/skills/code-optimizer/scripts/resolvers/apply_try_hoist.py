"""Apply the try/except hoists detected by detectors/try_hoist_analysis.py.

Deterministic, no LLM: hoists every safe per-iteration ``try/except``
around its loop. Nothing in `TryHoistPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_try_hoist.py --file <path> [--dry-run]
...
from detectors import try_hoist_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.TryHoistPlan) -> list[str]:
...
def apply_try_hoists(source: str, plan: analysis.TryHoistPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Hoist per-iteration try/except around the loop.",
    )
    ...
    if not plan.safe:
        print("No safe try-hoist candidates found.")
    ...
    try:
        new_source = apply_try_hoists(source, plan)
