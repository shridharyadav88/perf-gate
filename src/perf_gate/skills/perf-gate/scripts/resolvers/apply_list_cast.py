"""Apply the list-copy drops detected by detectors/list_cast_analysis.py.

Deterministic, no LLM: drops every safe ``list(...)`` copy around an
already-fresh iterable in loop position. Nothing in
`ListCastPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_list_cast.py --file <path> [--dry-run]
...
from detectors import list_cast_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.ListCastPlan) -> list[str]:
...
def apply_list_cast(source: str, plan: analysis.ListCastPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Iterate fresh temporaries directly, not via list(...).",
    )
    ...
    if not plan.safe:
        print("No safe list-copy candidates found.")
    ...
    try:
        new_source = apply_list_cast(source, plan)
