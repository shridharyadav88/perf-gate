"""Apply the range-len-to-enumerate conversions detected by detectors/enumerate_analysis.py.

Deterministic, no LLM: rewrites every safe ``for i in range(len(x))``
loop to ``for i, x_item in enumerate(x)`` (plus ``x[i]`` sites).
Nothing in `EnumeratePlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_enumerate.py --file <path> [--dry-run]
...
from detectors import enumerate_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.EnumeratePlan) -> list[str]:
...
def apply_enumerates(source: str, plan: analysis.EnumeratePlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Iterate with enumerate instead of range(len(...)).",
    )
    ...
    if not plan.safe:
        print("No safe enumerate candidates found.")
    ...
    try:
        new_source = apply_enumerates(source, plan)
