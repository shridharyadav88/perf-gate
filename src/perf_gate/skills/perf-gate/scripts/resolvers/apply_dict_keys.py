"""Apply the dict-keys simplifications detected by detectors/dict_keys_analysis.py.

Deterministic, no LLM: rewrites every safe ``for k in d.keys()`` /
``list(d)`` iterator to the bare dict name. Nothing in
`DictKeysPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_dict_keys.py --file <path> [--dry-run]
...
from detectors import dict_keys_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.DictKeysPlan) -> list[str]:
...
def apply_dict_keys(source: str, plan: analysis.DictKeysPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Iterate dicts directly, not via .keys() snapshots.",
    )
    ...
    if not plan.safe:
        print("No safe dict-keys candidates found.")
    ...
    try:
        new_source = apply_dict_keys(source, plan)
