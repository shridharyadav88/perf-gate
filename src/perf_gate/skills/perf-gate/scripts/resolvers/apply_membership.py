"""Apply the literal-membership conversions detected by detectors/membership_analysis.py.

Deterministic, no LLM: rewrites every safe ``x in [...]`` / ``x in (...)``
test with 3+ constant elements to a set literal. Nothing in
`MembershipPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_membership.py --file <path> [--dry-run]
...
from detectors import membership_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.MembershipPlan) -> list[str]:
...
def apply_memberships(source: str, plan: analysis.MembershipPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Test membership against set literals, not list literals.",
    )
    ...
    if not plan.safe:
        print("No safe membership candidates found.")
    ...
    try:
        new_source = apply_memberships(source, plan)
