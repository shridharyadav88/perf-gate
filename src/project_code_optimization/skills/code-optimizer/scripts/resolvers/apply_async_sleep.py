"""Apply the async-sleep conversions detected by detectors/async_sleep_analysis.py.

Deterministic, no LLM: rewrites every safe blocking ``time.sleep(...)``
inside an async function to ``await asyncio.sleep(...)``. Nothing in
`AsyncSleepPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_async_sleep.py --file <path> [--dry-run]
...
from detectors import async_sleep_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.AsyncSleepPlan) -> list[str]:
...
def apply_async_sleep(source: str, plan: analysis.AsyncSleepPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Await asyncio.sleep instead of blocking time.sleep.",
    )
    ...
    if not plan.safe:
        print("No safe async-sleep candidates found.")
    ...
    try:
        new_source = apply_async_sleep(source, plan)
