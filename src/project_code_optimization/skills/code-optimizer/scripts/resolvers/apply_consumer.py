"""Apply the consumer de-materializations detected by detectors/consumer_analysis.py.

Deterministic, no LLM: rewrites every safe ``func([...])`` / ``func(list(...))``
argument span to its generator (or stripped-iterable) form, and every safe
``len([...])`` call to ``sum(1 for ...)``. Nothing in
`ConsumerPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_consumer.py --file <path> [--dry-run]
...
from detectors import consumer_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.ConsumerPlan) -> list[str]:
    ...
    fresh_keys = {
        (c.func_name, c.kind, c.lineno, c.end_lineno, tuple(c.edits))
        for c in fresh.safe
    }
    errors = []
    for cand in plan.safe:
        key = (
            cand.func_name, cand.kind, cand.lineno, cand.end_lineno,
            tuple(cand.edits),
        )
        if key not in fresh_keys:
            errors.append(
                f"'{cand.kind}' in {cand.func_name} "
                f"(planned line {cand.lineno}): "
...
def apply_consumers(source: str, plan: analysis.ConsumerPlan) -> str:
    ...
    edits = [edit for cand in plan.safe for edit in cand.edits]
    try:
        new_source = span_edit.splice(source, edits)
    except ValueError as exc:
        raise PlanMismatchError(
            f"Overlapping candidate spans — refusing to apply: {exc}"
        ) from exc
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Feed single-pass consumers generators, not materialized lists.",
    )
    ...
    if not plan.safe:
        print("No safe consumer candidates found.")
    ...
    for cand in plan.safe:
        print(f"  '{cand.kind}' in {cand.func_name} (line {cand.lineno})")
    for func_name, reason in plan.skipped:
        print(f"  skipped in '{func_name}': {reason}")

    try:
        new_source = apply_consumers(source, plan)
    ...
    if args.dry_run:
        ...
        for cand in plan.safe:
            print(f"  line {cand.lineno} [{cand.kind}]: {cand.detail}")
        return
