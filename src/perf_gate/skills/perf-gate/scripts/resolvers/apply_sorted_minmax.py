"""Apply the sorted-extreme collapses detected by detectors/sorted_minmax_analysis.py.

Deterministic, no LLM: rewrites every safe ``sorted(...)[0/-1]`` extreme
to ``min(...)`` / ``max(...)``. Nothing in `SortedMinmaxPlan.skipped`
is ever touched.

Usage:
    python3 resolvers/apply_sorted_minmax.py --file <path> [--dry-run]
...
from detectors import sorted_minmax_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.SortedMinmaxPlan) -> list[str]:
...
def apply_sorted_minmax(source: str, plan: analysis.SortedMinmaxPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Take min/max directly instead of sorting first.",
    )
    ...
    if not plan.safe:
        print("No safe sorted-extreme candidates found.")
    ...
    try:
        new_source = apply_sorted_minmax(source, plan)
