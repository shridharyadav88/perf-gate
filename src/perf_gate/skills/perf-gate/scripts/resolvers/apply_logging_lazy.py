"""Apply the logging-lazy conversions detected by detectors/logging_lazy_analysis.py.

Deterministic, no LLM: rewrites every safe eager logging string to
lazy %-style arguments. Nothing in `LoggingLazyPlan.skipped` is
ever touched.

Usage:
    python3 resolvers/apply_logging_lazy.py --file <path> [--dry-run]
...
from detectors import logging_lazy_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.LoggingLazyPlan) -> list[str]:
...
def apply_logging_lazy(source: str, plan: analysis.LoggingLazyPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Pass logging arguments lazily instead of formatting eagerly.",
    )
    ...
    if not plan.safe:
        print("No safe lazy-logging candidates found.")
    ...
    try:
        new_source = apply_logging_lazy(source, plan)
