"""Apply the re.match-to-search conversions detected by detectors/rematch_search_analysis.py.

Deterministic, no LLM: rewrites every safe ``re.match(".*pat", ...)``
boolean test to ``re.search("pat", ...)``. Nothing in
`RematchSearchPlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_rematch_search.py --file <path> [--dry-run]
...
from detectors import rematch_search_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.RematchSearchPlan) -> list[str]:
...
def apply_rematch_search(source: str, plan: analysis.RematchSearchPlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Search directly instead of matching .* first.",
    )
    ...
    if not plan.safe:
        print("No safe rematch-search candidates found.")
    ...
    try:
        new_source = apply_rematch_search(source, plan)
