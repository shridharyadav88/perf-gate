"""Apply the comprehension rewrites detected by detectors/perf_comprehension_analysis.py.

Deterministic, no LLM: replaces every safe init+loop pair
(`out = []` + `for ...: out.append(...)`) with a single list-comprehension
or `list(...)` assignment. Nothing in `RewritePlan.skipped` is ever touched.

Usage:
    python3 resolvers/apply_perf_comprehension.py --file <path> [--dry-run]
...
from detectors import perf_comprehension_analysis as analysis  # noqa: E402
...
def verify_plan(source: str, plan: analysis.RewritePlan) -> list[str]:
...
def apply_rewrites(source: str, plan: analysis.RewritePlan) -> str:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Rewrite safe manual list-build loops as comprehensions (PERF401/402).",
    )
    ...
    if not plan.safe:
        print("No safe comprehension-rewrite candidates found.")
    ...
    for cand in plan.safe:
        print(
            f"  '{cand.var_name}' in {cand.func_name} "
            f"({cand.kind}, lines {cand.lineno}-{cand.end_lineno})"
        )
    for var_name, reason in plan.skipped:
        print(f"  skipped '{var_name}': {reason}")
    ...
    if args.dry_run:
        ...
        for cand in plan.safe:
            print(f"--- {cand.func_name} lines {cand.lineno}-{cand.end_lineno} ({cand.kind})")
            print(cand.indent + cand.replacement)
        return
